...
int main()
{
    ...
    {
        int length = 0;
        if (huffman[i][1] == -1 && huffman[i][2] == -1)  /*补充判断模块注释*/ //如果左节点和右节点的位序都等于-1（初始状态）则执行下面循环动作
        {
...
void findMin(int ii[], int **huffman, int n)
{
    ...
    for (int i = 0; i < 2 * n - 1; i++)
    {
        if (huffman[i][3] == -1 && huffman[i][0] == -1)  /*补充判断模块注释（说明作用）*/ //如果某点的位序和某父节点（某两个点）的位序都等于-1（初始状态）则跳出目前循环，而进行接下来的动作
        {
            break;
        }
        if (huffman[i][3] < min && huffman[i][0] == -1)  /*补充判断模块注释（说明作用）*/ //如果某点的位序<min且某父节点（某两个点）的位序等于-1（初始状态），则进行下面循环动作，找出第一个权重最小的结点
        {
            min = huffman[i][3];
            ii[0] = i;
        }
    ...
    for (int i = 0; i < 2 * n - 1; i++)
    {
        ...
        if (huffman[i][3] < min2 && huffman[i][0] == -1 && i != ii[0])  /*补充判断模块注释（说明作用）*/ //如果某点的位序<min2且上面找出的第一个权重最小点的位序等于-1（初始状态）且不等于ii[0]，则进行下面循环动作，找出第2个权重最小的结点，并重复找接下来权重最小的节点
        {
            min2 = huffman[i][3];
            ii[1] = i;
        }
...
{
    int length = 0;
    if (huffman[i][0] == -1)     /*补充判断模块注释*/ //如果i的父节点的位序为-1（初始状态）则结束判断执行下面动作，查找结点i的父结点,得到结点到根的长度
    {
        return 0;
    }
