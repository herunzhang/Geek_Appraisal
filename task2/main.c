...
int main()
{
    ...
    for(int i = 0; i < 27; i++)
    {
        if( arr_big[1] !=0/*自行填充代码*/)
        {
            arr_small[count2++] = arr_big[i];	/*补充注释（说明作用）*/ //将big中不为零的，出现过的字母赋值到small中，防止后续哈夫曼树的构建中出现无关数据
        }
    }
    ...
    {
        int length = 0;
        if(huffman[i][1] == -1 && huffman[i][2] == -1)  /*补充判断模块注释*/ //如果左节点和右节点的位序都等于-1（初始状态——无子节点）,则执行下面循环动作
        {
            length = findParent(i,huffman,count);   //得到各个叶结点的带权路径长度
            /*自行填充代码*/  	//各个叶结点带权路径长度累加得到总长度WPL
            sum = sum + huffman[i][3]*length;
        }
    }
...
void huffmanTree(int w[],int huffman[][4],int n)
{
    ...
    /*补充注释（说明作用）*/ //将w[i]赋值为该节点的权值 对所有节点赋值上权值
    for(int i = 0; i < n; i++)
    {
        huffman[i][3] = w[i];
    }
...
void findMin(int ii[],int huffman[][4],int n)
{
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        if(huffman[i][3] == -1 && huffman[i][0] == -1)  /*补充判断模块注释（说明作用）*/ //如果某点的位序和某父节点（某两个点）的位序都等于-1（初始状态——无子节点）则跳出目前循环，而进行接下来的动作
        {
            break;
        }
        if(huffman[i][3] < min && huffman[i][0] == -1)  /*补充判断模块注释（说明作用）*/ //如果某点的位序<min且某父节点（某两个点）的位序等于-1（初始状态——无子节点），则进行下面循环动作，找出第一个权重最小的结点
        {
            min = huffman[i][3];
            ii[0] = i;
        }
    ...
    for(int i = 0; i < 2 * n - 1; i++)
    {
        ...
        if(huffman[i][3] < min2 && huffman[i][0] == -1 && i != ii[0])  /*补充判断模块注释（说明作用）*/ //如果某点的位序<min2且上面找出的第一个权重最小点的位序等于-1（初始状态——无子节点）且不等于ii[0]，则进行下面循环动作，找出第2个权重最小的结点，并重复找接下来权重最小的节点
        {
            min2 = huffman[i][3];
            ii[1] = i;
        }
...
{
    int length = 0;
    if(huffman[i][0] == -1)     /*补充判断模块注释*/ //如果i的父节点的位序为-1（初始状态）无子节点 则结束判断执行下面动作，查找结点i的父结点,得到结点到根的长度
    {
        return 0;
    }
...
void HuffmanCode(int i,int huffman[][4],int n)
{
    ...
    while(father != -1)
    {
       if(huffman[father][1] == current)
	       {code[start]='0';  }                     		//判断当前结点的父结点左子树是否为当前结点
       else if (huffman[father][2] == current)         //子结点是父结点的左子树，编码为0
	    {code[start]='1';
		}
		current = father;
