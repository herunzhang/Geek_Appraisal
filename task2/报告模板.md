...
56 A 0000,B 0001,C 001,D 01,E 1
（2）EEBEACCCCAADACCBCCCCECCCEEECCCE
56 A 001,B 0001,C 1,D 0000,E 01
---

### 思考题

1. 当Huffman树的叶结点为n时，整个生成树的结点总数为何是2n-1？
因为父节点数量总比叶节点数少1，则整个生成树节点为n+n-1=2n-1
2. 该代码可以对大写字母的字符串进行huffman编码，如果要读小写字母字符串进行编码，应该如何修改？如果想要对同时具有大写和小写字母的字符串都进行编码呢（写出思路即可）？
在开始时if(str[i] == (char)(0x41 + j))中将0x41换为小写字母的ASCII码
大小写都有则都输入大小写的ASCII码
3. 使用画图工具画出上述两个Huffman生成树的生成过程

![](./Huffmantree%20%20process1.png)

![](./Huffmantree%20%20process2.png)